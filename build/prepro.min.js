    ...
    value: function showFrameDetails(frame) {
      for (var serviceName in frame) {
        ...
        if (!this.servicesViews_[serviceName]) {
          var ServiceViewClass = __webpack_require__(5)("./" + serviceName + '/view');
          if (!ServiceViewClass) {
            continue;
          }
    ...
    value: function setup(data) {
      ...
      for (var s in data) {
        if (services.indexOf(s) != -1) {
          var ServiceClass = __webpack_require__(19)("./" + s + '/service');
...
var map = {
	...
	"./flow/service": 21,
	"./openpose/service": 22,
	"./segmentation/service": 23,
	"./sift/service": 24,
	"./spectrogram/service": 25
};
function webpackContext(req) {
	return __webpack_require__(webpackContextResolve(req));
};
...
var Flow = function (_EventDispatcher) {
  _inherits(Flow, _EventDispatcher);
  ...
  _createClass(Flow, [{
    ...
    set: function set(video) {
      ...
      var drawNextFrame = function drawNextFrame() {
        ...
        ctx.drawImage(_this2.video_, 0, 0, ctx.canvas.width, ctx.canvas.height);
        _this2.currLoaded_++;
        if (_this2.currLoaded_ >= numFrames) {
          _this2.video_.removeEventListener('seeked', drawNextFrame);
          _this2.dispatch('ready', 'flow');
        } else {
          _this2.video_.currentTime = _this2.currLoaded_ / prepro.config.framerate;
        }
      };
...
var Segmentation = function (_EventDispatcher) {
  _inherits(Segmentation, _EventDispatcher);

  function Segmentation(data) {
    _classCallCheck(this, Segmentation);

    var _this = _possibleConstructorReturn(this, (Segmentation.__proto__ || Object.getPrototypeOf(Segmentation)).call(this));
